...
class Point:
    # Utilisation de variables privées comme demandé
    ...
    @x.setter
    def x(self, valeur: float):
        self.__x = valeur  # Corrigé : c'était un 'return' dans ton code d'origine
...
class Cercle:
    def __init__(self, rayon: float, centre: Point = None):
        ...
        self.rayon = rayon  # Passe par le setter pour vérifier si c'est négatif
    ...
    def intersection(self, camarade) -> bool:
        ...
        return distance <= self.__rayon + camarade.rayon
...
class Rectangle:
    def __init__(self, point: Point = None, longueur: float = 1, hauteur: float = 1, pointHautDroit: Point = None):
        if point is None:
            ...
            self.__pointBasGauche = point
            # Calcul propre avec les propriétés .x et .y
